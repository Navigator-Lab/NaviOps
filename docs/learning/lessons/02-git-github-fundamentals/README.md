...
`git` plumbing commands you learned in this lesson. Spec for `scripts/git-health-check.sh`:

- **Header:** `#!/usr/bin/env bash` + a one-line comment naming the script + `set -euo pipefail`.
- **Guard:** resolve the repo root with `git rev-parse --show-toplevel`; if it fails
  (not inside a repo), print an error to stderr and `exit 1`.
- **Report** (one line each, with a label):
  - current branch → `git rev-parse --abbrev-ref HEAD`
  - last commit → `git log -1 --format='%h %s (%ar)'`
  - working-tree status → `git status --short`
  - remotes → `git remote -v`
  - stash count → `git stash list | wc -l`
  - unpushed commits vs `origin/main` → `git log origin/main..HEAD --oneline`
- Make it executable (`chmod +x`).

Each command above is one you used earlier in this lesson — the exercise is wiring them
into a clean, labelled report in your own style, then committing it as your own work.
...
`LEARNING_STATE.md` flagged 3 partially-learned items from Lesson 01 for review here:
1. Directory `r` vs `x` distinction
2. Recursive `chmod`/`chgrp`/setgid/ACL "better fix" for shared directories
3. `chown` vs `chgrp` root requirements

Quiz Q5/Q8 here (merge conflicts, ACL-style fine-grained access) touch related ground,
but these 3 specific items weren't re-tested directly. **Carried forward to Lesson
03's reflection** rather than retrofitted here — Lesson 03 (Bash Scripting
Fundamentals) will write a script that touches file permissions
(`/scripts/user_audit.sh`-style), giving a natural hands-on point to revisit all three.
...
**Core**
- `git staging area vs working directory explained`
- `git fetch vs pull vs merge vs rebase`
- `git merge conflict resolution step by step`
- `gitignore not working already tracked file`

**Tools**
- `git bisect tutorial finding bad commit`
- `github branch protection rules pull request`
- `git filter-repo remove secret from history`

**Going further (future lessons)**
- `gitops workflow infrastructure as code`
- `ssh config host alias identityfile github`
